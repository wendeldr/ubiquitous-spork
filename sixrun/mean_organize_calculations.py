...
patient_hdf5_path = os.path.join(output_path, "mean_patients.h5")
ilae_hdf5_path = os.path.join(output_path, "mean_ilae.h5")
columns_hdf5_path = os.path.join(output_path, "mean_columns.h5")

# delete the files if they exist. prompt user to confirm
# prompt user to confirm
if input("Calculating mean values. This will delete the existing files (mean_patients.h5, mean_ilae.h5, mean_columns.h5). Are you sure? (y/n): ") != "y":
    print("Exiting...")
    exit()  
...
with h5py.File(columns_hdf5_path, "w") as hdf5_file:
    for col in columns:
        col_group = hdf5_file.create_group(str(col))
        col_group.create_group("upper")
        col_group.create_group("lower")

# Initialize ilae.h5
with h5py.File(ilae_hdf5_path, "w") as hdf5_file:
    for ilae in unique_ilae_groups:
        ilae_group = hdf5_file.create_group(str(ilae))
        for col in columns:
...
for pid in tqdm(pids, desc="Patients", leave=True):
    ...
    for col in tqdm(data[0].columns.levels[0].unique(), desc=f"Columns for {pid}", leave=False):
        ...
        measure = np.nanmean(measure, axis=0)
